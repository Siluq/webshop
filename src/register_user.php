...
function setFormData(){
    ...
    $password  = mysqli_real_escape_string($con, trim($_POST['field_user_password']));
    ...
    if( strlen($password) >= 5 && strlen($password) <= 32 ) {
        ...
        $password = dbp($_POST['field_user_password']);
    ...
    if($valid['check_firstname'] && $valid['check_lastname'] && $valid['check_email'] && $valid['check_password']){
        $query1 = $con->prepare('INSERT INTO `user` (`firstName`, `middleName`, `lastName`, `email`, `user_password`) VALUES (?,?,?,?,?);');
    if ($query1 === false) {
