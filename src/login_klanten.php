...
function logIn(){
  ...
  // Validate email
    if(isset($_POST['field_klanten_email'])){
      //checken of email is ingevuld
      //als de waarde true is
      $email= dbp($_POST['field_klanten_email']);
    }else{
        $email = "";
        echo "Email niet goed ingevuld";
    }
    //validate password
    if(isset($_POST['field_klanten_password'])){
      //checken of email is ingevuld
      //als de waarde true is
      $field_password= dbp($_POST['field_klanten_password']);
    }else{
        $field_password = "";
  ...
  $qry = $con->query("SELECT email,klanten_password FROM customer WHERE email = '{$email}';");
  if($qry === false){   
  ...
  exit(__LINE__);
    ...
    //password check;
      ...
      if (password_verify($field_password, $user['klanten_password'])) {
